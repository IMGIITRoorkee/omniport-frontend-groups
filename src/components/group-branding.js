...
class GroupBranding extends React.Component {
  ...
  removeCover = () => {
    this.props.ChangeActiveGroup(this.props.activeGroup.data.slug, 'brandingCover', {coverImage: null})
  }
  addCover = (e) => {
  ...
  removeLogo = () => {
    this.props.ChangeActiveGroup(this.props.activeGroup.data.slug, 'brandingLogo', {logo: null})
  }
  addLogo = (e) => {
    var formData = new FormData()
  ...
  render () {
    ...
    const {activeCover, activeLogo} = this.state
    const { data, hasEditRights, inEditMode } = activeGroup
    return (
      <Card fluid>
      <Dimmer.Dimmable dimmed={activeCover && hasEditRights} 
        ...
        {inEditMode==='brandingCover' ? <Loader active={inEditMode==='brandingCover'} /> :<React.Fragment>
          {data.coverImage && <Icon name='erase' bordered onClick={this.removeCover}/>}
        ...
        <Dimmer.Dimmable dimmed={activeLogo && hasEditRights}
        ...
        onMouseLeave={this.handleHideLogo}>
          ...
          <Dimmer active={activeLogo && hasEditRights} style={{borderRadius: '9999em'}}>
        {inEditMode==='brandingLogo' ? <Loader active={inEditMode==='brandingLogo'} /> :<React.Fragment>
          {data.logo && <Icon name='erase' bordered onClick={this.removeLogo}/>}
          <label htmlFor='logo'>
            <Icon name='pencil' />    
          </label>
          <input
        ...
        <Card.Content>
          <Grid
            ...
            columns={!data.socialInformation[0] || data.socialInformation[0].links.length===0 ? 2 : 3}
            stackable
          >
            ...
            {data.socialInformation[0] && data.socialInformation[0].links.length!==0 && (
              <Grid.Column textAlign='center'>
